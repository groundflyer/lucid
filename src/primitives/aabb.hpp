...
struct AABB_
{
    ...
    {
        Vec3 ret{};
        for(std::size_t i = 0; i < 3; ++i) ret[i] = (*this)[bool{idxs[i]}][i];
        return ret;
    }
};
...
template <template <typename, std::size_t> typename AABBContainer,
          template <typename, std::size_t>
...
{
    const auto& [s1, s2] = s;
    const real     s3    = fmod(2_r * (s1 + s2), 1_r);
    const unsigned shift = static_cast<unsigned>(300_r * fmod(s1 + s2 + s3, 1_r));
